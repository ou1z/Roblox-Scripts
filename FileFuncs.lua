return {
	OpenPrompt = function(callback, yield)
	...
	SavePrompt = function(text, yield)
		...
		local Refresh = Instance.new("ImageButton")
		local Bottom = Instance.new("Frame")
		local OpenSave = Instance.new("TextButton")
		local Cancel = Instance.new("TextButton")
		...
		SaveExplorer.Name = "SaveExplorer"
		SaveExplorer.Parent = game.CoreGui
		SaveExplorer.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

		Open.Name = "Open"
		Open.Parent = SaveExplorer
		Open.AnchorPoint = Vector2.new(0.5, 0.5)
		Open.BackgroundColor3 = Color3.fromRGB(32, 32, 32)
		Open.BorderSizePixel = 0
		Open.ClipsDescendants = true
		...
		OpenSave.Parent = Bottom
		OpenSave.BackgroundColor3 = Color3.fromRGB(32, 32, 32)
		OpenSave.BorderColor3 = Color3.fromRGB(181, 181, 181)
		OpenSave.Position = UDim2.new(0.689922452, 0, 0.290909111, 0)
		OpenSave.Size = UDim2.new(0.13333334, 0, 0.418181807, 0)
		OpenSave.Font = Enum.Font.SourceSans
		OpenSave.Text = "Save"
		OpenSave.TextColor3 = Color3.fromRGB(255, 255, 255)
		OpenSave.TextSize = 14.000
		...
		TextBox.BorderColor3 = Color3.fromRGB(156, 156, 156)
		TextBox.Position = UDim2.new(0.136977106, 0, 0.30883047, 0)
		TextBox.Size = UDim2.new(0.527915537, 0, 0.351575881, 0)
		...
		Selected.BackgroundTransparency = 1.000
		Selected.BorderColor3 = Color3.fromRGB(27, 42, 53)
		Selected.Position = UDim2.new(0.0285984538, 0, 0.329773486, 0)
		Selected.Size = UDim2.new(0.0993810669, 0, 0.272727281, 0)
		Selected.Font = Enum.Font.SourceSans
		...
		Workspace.BackgroundColor3 = Color3.fromRGB(32, 32, 32)
		Workspace.BorderSizePixel = 0
		Workspace.Size = UDim2.new(0.917358935, 0, 0, 25)
		...
		function Refresh(f)
			...
			--[[
				...
				local function selectfile()
					if table.find(SelectedFiles, v.Name) then -- Unselect --	
						table.remove(SelectedFiles, table.find(SelectedFiles, v.Name))
						unhover(true)
					else -- Select --
						SelectedFiles = {}
						for i,v in pairs(TemplateCache) do
				...
				if v.Type == "Folder" then
					template = SaveExplorer.Templates.FolderTemplate:Clone()
					template.Name = v.Name
					template.FolderName.Text = FileName
					template.Parent = FileList
					template.Visible = true
					table.insert(TemplateCache,template)
				elseif v.Type == "File" then
					template = SaveExplorer.Templates.FileTemplate:Clone()
					template.Name = v.Name
					template.FileName.Text = FileName
					template.Parent  = FileList
					template.Visible = true
		...
		Bottom.OpenSave.MouseButton1Click:Connect(function()
			local Selected = SelectedFiles[1]
			if not Selected then 
				writefile(Directory .. '/' .. Bottom.TextBox.Text, text)
				Open:TweenSize(UDim2.new(0,0,0,0),'Out','Linear',0.1,true,function()
					SaveExplorer:Destroy()
				end)	
				return end
