...
image.Image = rbxassetid3570695787
...
Send_Roundify_2px.Image = rbxassetid3570695787
...
image_2.ZIndex = 3
image_2.Image = rbxassetid3570695787
...
image_3.Image = rbxassetid3570695787
...
local function MICOMH_fake_script() -- Main.LocalScript 
	...
	local json_encode = function(tbl)
		local json = gameGetService(HttpService)JSONEncode(tbl)
		return json
	end
	local json_decode = function(json)
		local tbl = gameGetService(HttpService)JSONDecode(json)
		return tbl
	end
	...
	function newMessage(data)
		...
		local placeholder = script.Parent.MessageHolder.PlaceholderClone()
	...
	if not _G.client then
		...
		_G.client.OnMessageConnect(function(msg)
	...
	script.Parent.Minimize.MouseButton1ClickConnect(function()
		script.Parent.ParentTweenPosition(UDim2.new(-1.5,0,1.5,0), 'In', 'Linear', 0.5, true)
		script.Parent.Parent.Parent.OpenTweenPosition(UDim2.new(-0.006, 0,0.947, 0), 'In', 'Linear', 0.5, true)
	end)
	script.Parent.Parent.Parent.Open.MouseButton1ClickConnect(function()
		script.Parent.ParentTweenPosition(UDim2.new(0.5,0,0.5,0), 'Out', 'Linear', 0.5, true)
		script.Parent.Parent.Parent.OpenTweenPosition(UDim2.new(-1.5,0,1.5,0), 'Out', 'Linear', 0.5, true)
	end)
	...
	local Commands = {
		['.dex'] = function()
			if gameGetService'CoreGui'FindFirstChild'Dex' then
				gameGetService'CoreGui'.DexDestroy();
			end
			...
			local Dex = gameGetObjects(rbxassetid3567096419)[1]
			syn.protect_gui(Dex)
			Dex.Name = RandomCharacters(math.random(5, 20))
			Dex.Parent = gameGetService(CoreGui)
			...
			local function Load(Obj, Url)
				...
				local function LoadScripts(Script)
					...
					for i,v in pairs(ScriptGetChildren()) do
						LoadScripts(v)
					end
				end
		...
		['.remotespy'] = function()
			loadstring(gameHttpGet(httpsraw.githubusercontent.comou1zRoblox-ScriptsmasterRemoteSpy.lua))()
		end,
		['.quit'] = function()
			script.Parent.Parent.ParentDestroy()
		end
	}
	...
	Message.FocusLostConnect(function(enter)
		if not enter then return end;
		send()
	end)
	
	Message.ChangedConnect(function()
		if #Message.Text  150 then
			Message.Text = Message.Textsub(1,150)
		else
	...
	script.Parent.Send.MouseButton1ClickConnect(function()
		send()
	end)
	
	
	
	script.Parent.ClearMessages.MouseButton1ClickConnect(function()
		for i,v in pairs(script.Parent.MessageHolderGetChildren()) do
			if vIsA(Frame) and v.Name ~= 'Placeholder' then
				vDestroy()
			end
		end
	end)
	...
	--Minimize Color
	script.Parent.Minimize.MouseEnterConnect(function()
		gameGetService('TweenService')Create(script.Parent.Minimize.image, TweenInfo.new(0.5), {ImageColor3 = Color3.fromRGB(35,35,35)})Play()
	end)
	script.Parent.Minimize.MouseLeaveConnect(function()
		gameGetService('TweenService')Create(script.Parent.Minimize.image, TweenInfo.new(0.5), {ImageColor3 = Color3.fromRGB(40,40,40)})Play()
	end)
	
	
	--Message Box Color
	script.Parent.Message.MouseEnterConnect(function()
		gameGetService('TweenService')Create(script.Parent.Message.image, TweenInfo.new(0.5), {ImageColor3 = Color3.fromRGB(34,34,34)})Play()
	end)
	script.Parent.Message.FocusedConnect(function()
		gameGetService('TweenService')Create(script.Parent.Message.image, TweenInfo.new(0.5), {ImageColor3 = Color3.fromRGB(31,31,31)})Play()
	end)
	script.Parent.Message.FocusLostConnect(function()
		gameGetService('TweenService')Create(script.Parent.Message.image, TweenInfo.new(0.5), {ImageColor3 = Color3.fromRGB(39, 39, 39)})Play()
	end)
